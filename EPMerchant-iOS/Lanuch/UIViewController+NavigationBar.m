...
{
    //背景图
    UIImageView *imageV = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, 64)];
    _imageV = imageV;
    ...
    if (barStyle ==NavigationBarStyleVipCenter){
        imageV.backgroundColor = ColorWithRGB(23, 47, 99, 1);
         [self addLeftItemWithFrame:CGRectZero textOrImage:0 action:@selector(popToViewController) name:@"返回"];
    }
...
-(void)addNavigationBar:(NavigationBarStyle)barStyle title:(NSString *)title
{
    ...
    titleLabel.textColor = ColorWithRGB(218, 187, 132, 1);
    titleLabel.text = title;
    if (barStyle ==1) {
        titleLabel.font = [UIFont fontWithName:@"ShiShangZhongHeiJianTi" size:20];
    }else{
...
//        imageV.userInteractionEnabled = YES;
        ...
        label.textColor = ColorWithRGB(216, 186, 131, 1);
...
{
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    [button setFrame:CGRectMake(ScreenWidth-BTNW, 20, BTNW, 44)];
    [self.view addSubview:button];
    if (action!=nil) {
        [button addTarget:self action:action forControlEvents:UIControlEventTouchUpInside];
    }
